...
contract Land {
    ...
    function addLandInspector(address _addr,string memory _name, uint _age, string memory _designation,string memory _city) public     onlyOwner // Use a modifier to simplify ownership checks
    returns (bool) 
{
    require(!RegisteredInspectorMapping[_addr], "Inspector already registered");
    ...
    function registerUser(string memory _name, uint _age, string memory _city,string memory _aadharNumber, string memory _panNumber, string memory _email
    ) public {
